...
function drawMap(ix,iy)  {
   ...
   for(var i =0; i< map_arr_width; i++)      {
        ...
        for(var j =0; j< map_arr_height; j++)      {
                   ...
                   if(!isBGImg(map_all[i*map_arr_width+j])  && isEndItem(map_all[i*map_arr_width+j],i*map_arr_width+j)  && px<900)  {
...
<div  id="map" style="z-index:0;border: 5px solid #000;width:940px;height: 512px; left:0; top:0; position: absolute; background:url(<?php echo $canvasBg?>) no-repeat;background-position:-512px -570px;-webkit-background-size:2048px 1332px">
...
<div title="gallery-holder" style="width:840px;z-index:1000">
	<div class="gallery" id="gallery1">
     ...
     for($i=0;$i<count($useritems);$i++)  {
         ...
         if($isIPhone)
             echo "<div class='item'><div class='box box1'><img id='".$itemID."' src='".$tar_img_file."' style='width:$width;height:$height' ontouchend=endSelectedItem() ontouchstart=setSelectedItem_sa(this.id,".$imgoff_str.",'".$sub_items_str."','".$loc_str."')></div></div>";
         else
